...
public class Grid<T> {
	...
	/**
	 * 0-3, 0 correspond to up right, 1 corrspond right, down (as in the map will
	 * add new space up/downs)
	 */
	...
	public Grid<T> generateGridSmall(int width, int height) {
		int currentWidth = 0;
		int currentHeight = 0;
		
		Grid<T> grid = new Grid<T>(8, 8, this.getItems(), this.seed);
		...
		while (currentWidth < width || currentHeight < height) {
			...
			grid = new Grid<T>(currentWidth, currentHeight, grid);
			try {
				while (true) {
	...
	public Grid<T> createGridX(Grid<T> grid) {
		Grid<T> grid2 = new Grid<T>(grid.getX() + 1, grid.getY(), grid.getItems(), grid.getSeed());
		
		for(int i = 0; i < grid.getY(); i++) {
			try {
				grid2.add(grid.getGrid()[i][grid.getX()-1], 0, i);
	...
	public Grid<T> createGridY(Grid<T> grid) {
		Grid<T> grid2 = new Grid<T>(grid.getX(), grid.getY() + 1, grid.getItems(), grid.getSeed());
		
		for(int i = 0; i < grid.getX(); i++) {
			try {
				grid2.add(grid.getGrid()[grid.getY()-1][i], i, 0);
			} catch (ItemExistsException e) {
				e.printStackTrace();
			}
		}
	...
	//oldgrid + newgrid = [old + new]
	public Grid<T> fillGridX(Grid<T> grid) {
		Grid<T> grid2 = createGridX(grid);
				
		grid = new Grid<T>(grid.getX() + 64, grid.getY(), grid);
		
		
		for(int x = 1; x < grid2.getX(); x++) {
			for(int y = 0; y < grid2.getY(); y++) {
				
				try {
					System.out.println((grid2.getX()-1) + x - 1);
					grid.add(grid2.getGrid()[y][x], (grid2.getX()-1) + x - 1, y);
	...
	public Grid<T> fillGridY(Grid<T> grid) {
		Grid<T> grid2 = createGridY(grid);
				
		grid = new Grid<T>(grid.getX(), grid.getY() + 64, grid, 1);
		
		for(int y = 1; y < grid2.getY(); y++) {
			for(int x = 0; x < grid2.getX(); x++) {
				
				try {
					grid.add(grid2.getGrid()[y][x], x, (grid.getX()/2) + y - 1);
	...
	public void forceAdd(Item<T> item, int x, int y){
		gridArray[y][x] = new Item<T>(item);
		gridArray[y][x].setXY(x, y);
		gridArray[y][x].setCollapse(0);
	}
	...
	public void remove(int x, int y) {
		...
		/*
		 * boolean[] dirCheck = testDirection(x, y); if (dirCheck[0] && (gridArray[y +
		 * 1][x].getData() != null)) { gridArray[y][x] = new Item<T>(gridArray[y +
		 * 1][x].getData(), x, y + 1, itemList.size() - 1, t); calculateSurroundings(x,
		 * y + 1);
		 * 
		 * } if (dirCheck[1] && (gridArray[y][x+1].getData() != null)) { gridArray[y][x
		 * + 1] = new Item<T>(gridArray[y][x+1].getData(), x + 1, y, itemList.size() -
		 * 1, t); calculateSurroundings(x + 1, y);
		 * 
		 * } if (dirCheck[2] && (gridArray[y - 1][x].getData() != null)) { gridArray[y -
		 * 1][x] = new Item<T>(gridArray[y - 1][x].getData(), x, y - 1, itemList.size()
		 * - 1, t); calculateSurroundings(x, y-1);
		 * 
		 * } if (dirCheck[3] && (gridArray[y][x - 1].getData() != null)) {
		 * gridArray[y][x - 1] = new Item<T>(gridArray[y][x - 1].getData(), x - 1, y,
		 * itemList.size() - 1, t); calculateSurroundings(x - 1, y); }
		 * 
		 */
		boolean[] dirCheck = testDirection(x, y);
		if (dirCheck[0]) {
